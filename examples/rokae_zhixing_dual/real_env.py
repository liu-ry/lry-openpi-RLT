"""Real environment for dual Rokae AR arms with Zhixing grippers.
...
class DualRokaeZhixingRealEnv:
    def __init__(
        ...
        *,
        ...
        if left_arm_remote_ip == right_arm_remote_ip:
            raise ValueError(
                "left_arm_remote_ip and right_arm_remote_ip are identical. "
                "Dual-arm rollout requires two distinct Rokae robot IPs."
            )
        if (
        ...
        self.arms = _utils.DualRokaeArm(
            left=_utils.RokaeSDKArm(
                remote_ip=left_arm_remote_ip,
                local_ip=left_arm_local_ip,
                name="left",
                sdk_python_dir=rokae_sdk_python_dir,
            ),
            right=_utils.RokaeSDKArm(
                remote_ip=right_arm_remote_ip,
                local_ip=right_arm_local_ip,
                name="right",
                sdk_python_dir=rokae_sdk_python_dir,
            ),
        )
    ...
    def _parse_reset(
        ...
        if reset_joint_positions is not None:
            ...
            raise ValueError(
                ...
                f"for dual Rokae arms, got {arr.size}"
            )
        ...
        left_pose = np.asarray(left_reset_end_pose or constants.LEFT_RESET_END_POSE, dtype=np.float32)
        right_pose = np.asarray(right_reset_end_pose or constants.RIGHT_RESET_END_POSE, dtype=np.float32)
    ...
    def send_action(self, action: np.ndarray) -> None:
        ...
        if action.size != constants.DUAL_ACTION_DIM:
            raise ValueError(f"dual Rokae action must be {constants.DUAL_ACTION_DIM}D, got {action.size}")
        if not np.all(np.isfinite(action)):
            self.arms.stop()
            self.left_gripper.stop()
            self.right_gripper.stop()
            raise ValueError("dual Rokae action contains NaN or Inf")
    ...
    def _move_to_reset(self) -> None:
        if self._left_reset is not None and self._right_reset is not None:
            ...
            if not self._allow_zero_reset_pose and np.allclose(reset_q, 0.0, atol=1e-6):
                raise RuntimeError(
                    "Refusing to move both Rokae arms to all-zero reset joints. "
            ...
            raise RuntimeError("No valid Rokae reset target configured")
...
def make_real_env(**kwargs) -> DualRokaeZhixingRealEnv:
    return DualRokaeZhixingRealEnv(**kwargs)
